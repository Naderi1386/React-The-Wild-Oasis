...
interface SalesChartPropType {
  bookings: DataBookingSelectedType[];
  numDays: string;
}

const SalesChart = ({ bookings, numDays }: SalesChartPropType) => {
  ...
  const data = allDates.map((date) => {
    return {
      ...
      totalSales: bookings
        ...
        .reduce((now: number, next: DataBookingSelectedType) => {
          return now + next.totalPrice;
        }, 0),
      extrasSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
        .reduce((now: number, next: DataBookingSelectedType) => {
          return now + next.extrasPrice;
        }, 0),
    };
  });
